...
def main() -> None:
    ...
    temporal_samples = build_temporal_training_samples(prepared["train"], sequence_length=12, stride=6)
    model = LSTMAnomalyModel(epochs=10, batch_size=32, hidden_size=48)
    model.train(temporal_samples)

    model_path = Path("outputs/models/temporal_autoencoder.pt")
    model.save(model_path)

    calibration_scores = score_temporal_rows(prepared["calibration"], model, sequence_length=12, stride=6)
    calibration = ThresholdCalibrator(percentile=97.5).calibrate(calibration_scores)

    nominal_scores = score_temporal_rows(prepared["nominal_eval"], model, sequence_length=12, stride=6)
    anomalous_scores = score_temporal_rows(prepared["anomalous_eval"], model, sequence_length=12, stride=6)
    ...
    metrics_path = evaluation_dir / "temporal_model_metrics.json"
    comparison_path = evaluation_dir / "temporal_model_score_comparison.csv"
    histogram_path = evaluation_dir / "temporal_model_score_histogram.png"
    threshold_path = Path("outputs/calibration/temporal_threshold.json")
    ...
    metrics_path.write_text(
        json.dumps(
            {
                "training_samples": int(len(temporal_samples)),
                "threshold": calibration.threshold,
                "calibration": {
                    "percentile": calibration.percentile,
                    "mean_score": calibration.mean_score,
    ...
    _save_histogram(
        ...
        threshold=calibration.threshold,
        title="Temporal Model Score Distribution",
        output_path=histogram_path,
    )

    print(f"Saved temporal model to {model_path}")
    print(f"Saved temporal threshold to {threshold_path}")
    print(f"Saved temporal metrics to {metrics_path}")
    print(f"Saved temporal score comparison to {comparison_path}")
    print(f"Saved temporal score histogram to {histogram_path}")
